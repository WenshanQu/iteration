...
```{r}
output = vector("list", length = 4) ## create a blank list
...
listcol_df = 
  tibble(
    name = c("a", "b", "c", "d"),
    norms = list_norms   ## it's a weird colomn, but it's just a data frame
  )


## We can manipulate the df
listcol_df %>% 
  filter(name == "a")
...
## We can use functions to deal with df
mean_and_sd(listcol_df$norms[[4]])
map(listcol_df$norms, mean_and_sd)


## Add a summary colomn to the df
listcol_df %>% 
...
## Do linear regression! Relationship between tmax and tmin.
lm(tmax ~ tmin, data = weather_nested$data[[1]])
```
Linear Regression with `Function`!
```{r}
...
Learn **`unnest()`** function! useful function!

copy function from last class
```{r}
